...
const SoundPreference: React.FC<SoundPreferenceProps> = ({ onCompleted }) => {
  ...
  const [showIntro, setShowIntro] = useState(false); // controls animation
  ...
  const handleNext = () => {
    setShowIntro(true); // trigger intro instead of finishing immediately
  };
  ...
  return (
    <>
      {/* STORY INTRO OVERLAY */}
      {showIntro && (
        <StoryIntro onComplete={onCompleted} />
      )}

      {/* MAIN UI (still rendered underneath) */}
      <Flex
        h="100vh"
        bg="gray.50"
        _dark={{ bg: 'gray.800' }}
