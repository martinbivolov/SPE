...
const LifestyleSelectionSection: React.FC<LifestyleSelectionSectionProps> = ({
  ...
  // Groups that contain at least one quotes-type question
  const quotesGroups = useMemo(
    () =>
      groups
        .map((group) => ({
          ...group,
  ...
  const currentGroup = quotesGroups[currentGroupIndex];
  const isLastGroup = currentGroupIndex === quotesGroups.length - 1;
    ...
    >
      ...
      {currentGroup && (
        <Text
          ...
          mb={4}
        >
          {translateGroup(currentGroup.id, currentGroup.title)} &mdash; {t('lifestyle.step', { current: currentGroupIndex + 1, total: quotesGroups.length })}
        </Text>
      )}
